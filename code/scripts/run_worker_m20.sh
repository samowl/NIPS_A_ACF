#!/bin/bash
# M20 worker: PASS-A + PASS-B for the BraTS multimodal (T1c/T2w/FLAIR) matrix.
#
# Usage (inside a raw-data environment where BraTS is available):
#   bash scripts/run_worker_m20.sh WORKER_ID N_WORKERS [GPU_ID]
#
# Slices jobs_m20.txt by line index modulo N_WORKERS. Two verbs:
#
#   EXTRACT brats_wt_multimodal {fm}            -> extract_features.py
#   HEAD    brats_wt_multimodal {fm} {seed}     -> train_head_cached.py
#
# HEAD lines block (poll-and-wait, capped at ~30 min) until the matching
# EXTRACT has written
# results/feature_cache/brats_wt_multimodal/{fm}/manifest.json. EXTRACT
# and HEAD are both skipped if their expected output already exists, so
# the worker is fully resume-safe and idempotent.
#
# IMPORTANT: M20 requires a raw-data environment because the BraTS NIfTI
# corpus lives at data/raw/brats2024_gli/. The first
# EXTRACT call on a given (subject) builds the per-subject npz cache
# under <brats_root>/_cache_multimodal/, which is disjoint from the
# unimodal _cache/ used by brats_wt.
#
# GPU_ID (optional) selects which physical GPU to bind via
# CUDA_VISIBLE_DEVICES. Default uses the Singularity-visible device 0.
#
# Logs append to results/logs/worker_m20_${WORKER_ID}.log; final status
# line lands in results/logs/worker_m20_${WORKER_ID}.status.
...
JOBS_FILE="${REPO_ROOT}/jobs_m20.txt"
LOG_DIR="${REPO_ROOT}/results/logs"
STATUS_FILE="${LOG_DIR}/worker_m20_${WORKER_ID}.status"
LOG_FILE="${LOG_DIR}/worker_m20_${WORKER_ID}.log"
mkdir -p "${LOG_DIR}"
...
log "worker_m20 ${WORKER_ID} complete: ran=${ran} skipped=${skipped} failed=${failed}"
