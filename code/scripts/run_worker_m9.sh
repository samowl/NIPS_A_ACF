#!/bin/bash
# M9 worker (cross-architecture random-init: ConvNeXt-T + ResNet-50 on
# acdc_lv + riga_cup, 2 FMs x 2 tasks x 8 seeds = 32 cells). Reads
# jobs_m9.txt and dispatches each line to scripts/train_adapted.py.
#
# This is a thin sibling of run_worker_adapt.sh that swaps:
#   - JOBS_FILE: jobs_adaptation.txt -> jobs_m9.txt
#   - LOG/STATUS prefix: worker_adapt_* -> worker_m9_*
#
# All other behaviour (resume safety, slice-modulo dispatch, ADAPT line
# parsing, train_adapted.py invocation, FMPOOL_TASKS_ALLOW filter) is
# identical to run_worker_adapt.sh.
#
# Job line format (8 fields, identical to jobs_adaptation.txt):
#   ADAPT {task} {fm} {seed} {regime} {lora_rank|-} {lora_blocks|-} {epochs}
#
# For M9 every line has regime=rand_init, lora_rank=-, lora_blocks=-,
# epochs=30. The CNN rand_init path is supported by train_adapted.py
# (build_random_encoder branches at lines 244-255).
#
# Usage (inside a tmux Singularity session):
#   bash scripts/run_worker_m9.sh WORKER_ID N_WORKERS [GPU_ID]
#
# Optional env:
...
JOBS_FILE="${REPO_ROOT}/jobs_m9.txt"
LOG_DIR="${REPO_ROOT}/results/logs"
STATUS_FILE="${LOG_DIR}/worker_m9_${WORKER_ID}.status"
LOG_FILE="${LOG_DIR}/worker_m9_${WORKER_ID}.log"
mkdir -p "${LOG_DIR}"
...
log "starting M9 WORKER_ID=${WORKER_ID} N_WORKERS=${N_WORKERS} GPU=${CUDA_VISIBLE_DEVICES:-default}"
...
log "worker_m9 ${WORKER_ID} complete: ran=${ran} skipped=${skipped} filtered=${filtered} failed=${failed}"
