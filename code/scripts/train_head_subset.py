#!/usr/bin/env python
"""M19: PASS-B head trainer with training-set-fraction subsampling.

Mirrors :mod:`scripts.train_head_cached` but adds ``--train-frac`` to control
how much of the cached training set is used for head fitting. The same cached
features (PASS-A output) are reused; only PASS-B (head training) sees a
seeded random subset of training indices. Test split is unchanged.

Designed for M19: 5 FM x 1 task (brats_wt) x 1 seed x 4 fractions
{0.1, 0.25, 0.5, 1.0} = 20 cells.

Output schema additions over ``train_head_cached``:
    - ``train_frac`` (float)
    - ``n_train_used`` (int)
    - ``n_train_total`` (int)
    - ``train_subset_indices`` (list[int]) - recorded for reproducibility
    - ``m19_schema_tag`` (str) = "subset_v1"

Exit codes
----------
0 success, 3 cache missing/invalid, 4 manifest mismatch, 5 frac out of range.
"""
...
logger = logging.getLogger("fmpool.train_head_subset")
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="M19 train-set fraction PASS-B head trainer (cached features)"
    )
    ...
    p.add_argument(
        ...
        type=float,
        required=True,
        help="Fraction of training cache to use (0 < f <= 1.0)",
    )
...
def _seeded_subset(n_total: int, frac: float, seed: int) -> torch.Tensor:
    """Return a deterministic permutation-based subset of training indices.

    Uses a CPU generator seeded with ``seed`` (XORed with a fixed constant to
    decouple it from the per-epoch shuffle generator in ``_train_head``) to
    draw a permutation of ``range(n_total)``; the first ``round(frac * n)``
    entries (>=1) are returned.
    """
    if not (0.0 < frac <= 1.0):
        raise ValueError(f"train_frac must be in (0, 1], got {frac}")
    n_use = max(1, int(round(n_total * float(frac))))
    n_use = min(n_use, n_total)
    gen = torch.Generator(device="cpu").manual_seed(int(seed) ^ 0xA5A5_5A5A)
    perm = torch.randperm(n_total, generator=gen)
    return perm[:n_use].clone()
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not (0.0 < args.train_frac <= 1.0):
        logger.error("--train-frac out of range (0, 1]: %s", args.train_frac)
        return 5
    ...
    # Keep the full train cache UNPINNED until subset selection: pinning the
    # full tensor before index_select doubles host pinned memory on BraTS-
    # scale caches. Test-side tensors are small and can pin immediately.
    train_feats_full = train_blob["feats"]
    train_masks_full = train_blob["masks"]
    ...
    n_train_total = int(train_feats_full.shape[0])
    if train_feats_full.shape[1] != feature_dim or train_feats_full.shape[2] != feat_h:
        logger.error(
            "feature shape mismatch with manifest: tensor %s vs manifest D=%d hw=%dx%d",
            tuple(train_feats_full.shape), feature_dim, feat_h, feat_w,
        )
        return 4

    subset_idx = _seeded_subset(
        n_total=n_train_total, frac=float(args.train_frac), seed=int(args.seed)
    )
    n_train_used = int(subset_idx.numel())
    if n_train_used == n_train_total:
        # Full fraction: skip the index_select copy entirely.
        train_feats = train_feats_full
        train_masks = train_masks_full
    else:
        train_feats = train_feats_full.index_select(0, subset_idx)
        train_masks = train_masks_full.index_select(0, subset_idx)
    if pin:
    ...
    ckpt_dir = (
        ...
        / "results" / "checkpoints_m19" / f"frac{args.train_frac}"
        / args.task / args.fm
    )
    ...
    if len(test_ids) != n_test:
        logger.error(
            "schema parity mismatch: ids=%d per_case=%d", len(test_ids), n_test
        )
        return 4
    payload = {
        "task": args.task,
        ...
        "train_subset_indices": [int(x) for x in subset_idx.tolist()],
        "n_test": int(per_case.numel()),
        "test_ids": list(test_ids),
        "per_case_dice": [float(x) for x in per_case.tolist()],
        "mean_dice": float(per_case.mean().item())
        if per_case.numel()
        else float("nan"),
        ...
        "m19_schema_tag": M19_SCHEMA_TAG,
    }
    body = json.dumps(payload, indent=2).encode("utf-8")
    _atomic_write_bytes(out_path, body)

    logger.info(
        "wrote %s frac=%.3f n_used=%d/%d n_test=%d mean_dice=%.4f train_s=%.1f",
        out_path,
        payload["train_frac"],
        payload["n_train_used"],
        payload["n_train_total"],
        payload["n_test"],
        payload["mean_dice"],
        train_elapsed,
    )
