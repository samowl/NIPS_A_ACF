#!/bin/bash
# M22 worker: MC-dropout PASS-B head trainer over jobs_m22.txt.
#
# Usage (inside a tmux Singularity session):
#   bash scripts/run_worker_m22.sh WORKER_ID N_WORKERS [GPU_ID]
#
# Slices jobs_m22.txt by line index modulo N_WORKERS. The single supported
# verb is:
#
...
#
# Logs append to ``results/logs/worker_m22_${WORKER_ID}.log``.
...
JOBS_FILE="${REPO_ROOT}/jobs_m22.txt"
LOG_DIR="${REPO_ROOT}/results/logs"
STATUS_FILE="${LOG_DIR}/worker_m22_${WORKER_ID}.status"
LOG_FILE="${LOG_DIR}/worker_m22_${WORKER_ID}.log"
mkdir -p "${LOG_DIR}"
...
while IFS= read -r line; do
    ...
    out_dir="${REPO_ROOT}/results/per_case_dice_m22/p${rate}/${task}/${fm}"
    out_json="${out_dir}/seed_${seed}.json"
    cache_dir="${CACHE_ROOT}/${task}/${fm}"
    manifest_path="${cache_dir}/manifest.json"

    if [[ -f "${out_json}" ]]; then
        log "SKIP idx=${idx} task=${task} fm=${fm} seed=${seed} rate=${rate} (json present)"
        skipped=$((skipped + 1))
        continue
    fi
    if [[ ! -f "${manifest_path}" ]]; then
        log "FAIL idx=${idx} task=${task} fm=${fm} seed=${seed} rate=${rate} cache_missing"
        failed=$((failed + 1))
        continue
    fi

    log "RUN idx=${idx} task=${task} fm=${fm} seed=${seed} rate=${rate}"
    start_ts=$(date +%s)
    "${PYTHON}" "${REPO_ROOT}/scripts/train_head_mc_dropout.py" \
    ...
    if [[ $rc -eq 0 && -f "${out_json}" ]]; then
        md=$(python3 -c "import json;d=json.load(open('${out_json}'));print('mc=%.4f std=%.4f H=%.4f'%(d.get('mc_mean_dice',float('nan')),d.get('mc_std_dice',float('nan')),d.get('mc_predictive_entropy',float('nan'))))" 2>/dev/null || echo NA)
...
log "worker_m22 ${WORKER_ID} complete: ran=${ran} skipped=${skipped} failed=${failed}"
