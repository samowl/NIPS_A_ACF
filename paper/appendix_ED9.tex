...
chronology. It starts with diagnostic readouts, then documents primary
estimators, uncertainty, task-level sources, and estimator sensitivity. Middle
sections collect same-backbone perturbations, segmentation-pipeline scope
checks, distribution-shift and held-out stress tests, operational metrics, and
$M_{\mathrm{eff}}$. Later sections collect mechanism diagnostics and additional
scope tables, and the appendix ends with the release manifest and artifact
metadata. Secondary rows are retained only when they qualify or explain a paper
claim, and their captions state when they use non-primary pools or splits.
Unless explicitly marked primary or sourced from the primary
...
\begin{table}[!htbp]
  \caption{\textbf{Appendix guide.} Blocks describe the actual order of sections
  below; cross-references point to the detailed tables rather than repeating
  them in the main paper.}
  \label{tab:appendix-guide}
  \centering\footnotesize
  \begin{tabularx}{\linewidth}{p{0.27\linewidth}p{0.27\linewidth}Y}
    \toprule
    Block & Sections (in order) & Evidence role \\
    \midrule
    Estimators and provenance &
    App.~\ref{sec:confidence-app}, \ref{sec:pixerror-app}, \ref{sec:bootstrap}, \ref{app:audit-sensitivity}, \ref{app:quality-controlled-pairs} &
    Diagnostic readouts, pixel overlap, case/hierarchical bootstrap, functional-floor/ICC/permutation sensitivity, pair-quality OLS. \\
    Operational metrics &
    App.~\ref{app:tau-sweep}, \ref{sec:kvasir-app}, \ref{sec:matched-app}, \ref{app:medsam-note} &
    Failure-event $\tau$-sweep, conditional recovery, Kvasir secondary split, matched-$M$ lift, MedSAM probe details. \\
    Same-backbone controls &
    App.~\ref{app:subject-cluster}, \ref{app:brats-ablations}, \ref{sec:adaptation}, \ref{sec:samebb} &
    Subject clustering, BraTS sparse caveat, ACDC architecture caveat / random-init / LoRA scope, bootstrap-bagging. \\
    Adaptation \& distribution shift &
    App.~\ref{app:distshift}, \ref{app:unet}, \ref{app:nnunet}, \ref{app:w1-scope} &
    RIGA cross-vendor, UNet-skip decoder, nnU-Net fold reslicing + case-identical, segmentation-recipe scope (loss/aug, length, dimensionality). \\
    Held-out and meta-analyses &
    App.~\ref{app:heldout}, \ref{app:lever}, \ref{app:lofo}, \ref{app:lift-ceiling}, \ref{app:3dfm}, \ref{app:matched-recipe} &
    Held-out 5-task stress test, lever index, leave-one-family-out, lift-ceiling support, 3D pilot, matched-recipe adaptation scope. \\
    Mechanism diagnostics &
    App.~\ref{app:diffleak}, \ref{app:meff}, \ref{app:rand-effects}, \ref{app:arch-3task}, \ref{app:cross-checkpoint} &
    Item-difficulty residualisation (cross-fitted + holdout), $M_{\mathrm{eff}}$, random-effects representation, ViT cross-pretraining, cross-checkpoint same-family. \\
    Additional scope diagnostics &
    App.~\ref{app:matrix-extra}, \ref{app:m3}, \ref{app:m9}, \ref{app:m14}, \ref{app:m15}, \ref{app:m19}, \ref{app:m20}, \ref{app:m22} &
    Decoder capacity, random-init CNN, loss/aug sensitivity, fold CV, BraTS train-size, BraTS multimodal, MC dropout. \\
    Artifact record &
    App.~\ref{app:release-manifest} &
    Bundle manifest, claim-to-artifact map, compute scope, trace inventory. \\
    \bottomrule
  \end{tabularx}
\end{table}

\section{Diagnostic readouts and scope}
\label{sec:confidence-app}

This section records diagnostic readouts that clarify scope without adding new
primary claims.
...
Case-only bootstrap intervals match the Case CI column of Table~\ref{tab:main} exactly (the case-only resampling uses the same fixed family/seed structure as the per-row case CI), so we do not retabulate them here. The remainder of this section reports BraTS pool reconciliation and the slice-vs-subject sampling caveats.

\paragraph{BraTS foreground pool reconciliation.} The 5-FM, 8-FM, and 8-FM-alt-estimator BraTS values cited elsewhere differ because they use disjoint analytic pools (5-FM: a subset of the 11-bundle source pool; 8-FM: a completed-only subset; alt-estimator: scoring with a different family-grouping convention). The principled headline is the post-floor 10-FM value because it uses the Dice${\geq}0.30$ functional floor and the Table~\ref{tab:main} same/cross definition consistent with every other primary row. The primary BraTS foreground result is $\Delta{=}0.315$ [0.302, 0.329] (case-level CI) from the 11-bundle source pool, with 10 bundles retained after the Dice$\geq$0.30 floor (BiomedCLIP, CLIP-B/16, CLIP-L/14, ConvNeXt-T, DeiT-B/16, DINOv2-B/S, EfficientNet-B0, MAE-B/16, ResNet-50; ResNet-18 is the only bundle below the floor). Source: \texttt{results/\_merged/paper\_table.json}. Subset diagnostics without released aggregate files are omitted from the released numeric evidence. The value reported throughout the main text and abstract is therefore the Table~\ref{tab:main} value.

All gap CIs strictly exclude zero under the case-level bootstrap, matching the family/seed-level CIs in the main text.

\paragraph{Per-rater RIGA Cup sensitivity (legacy non-replayable pool).} A non-replayable legacy sensitivity pool (containing RETFound traces not in the released artifact) repeated the symmetric multi-seed analysis using each of RIGA's six individual rater masks; per-rater Pearson within--cross gaps were $0.46$--$0.53$ across all six raters, with the range across raters an order of magnitude smaller than the within-vs-cross gap itself. The released Table~\ref{tab:main} RIGA Cup row remains the bundle-replayable evidence.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    Kvasir    & 44 & 0.261 & 0.333 & 0.261$^{\flat}$ \\
    ACDC LV   & 40 & 0.321 & 0.401 & 0.242--0.355 \\
    BraTS foreground & 40 & 0.315 & 0.404 & 0.315--0.362 \\
    RIGA Cup  & 36 & 0.473 & 0.539 & 0.384--0.529 \\
    RIGA Disc & 44 & 0.638 & 0.716 & 0.638$^{\flat}$ \\
    \bottomrule
    \multicolumn{5}{p{0.84\linewidth}}{\footnotesize All five rows have observed-gap-exceeded
    rank fraction $0.001$ at the 0.30 floor. $^{\flat}$ Value is constant across all valid floors in $\{0,0.20,0.25,0.30,0.35,0.40\}$.} \\
  \end{tabular}
\end{table}
...
$0.343$--$0.437$; the smallest value occurs when the largest-gap RIGA Disc row
is held out. These checks do not add new cases or new model training. They
instead reduce the risk that the primary sign is an artifact of the exact
functional floor, the Pearson statistic, or arbitrary family labels.

\paragraph{CLIP-L/14 inclusion sensitivity (ACDC LV).}
Because CLIP-L/14 passes the Dice${\geq}0.30$ floor on the primary ACDC split
but collapses on the secondary 8-FM split (Dice $0.007$;
Appendix~\ref{sec:acdc-detail}), we recompute the primary ACDC row with all
\texttt{clip\_vitl14} traces removed before any other step. With CLIP-L/14
excluded the source pool drops to $10$ bundles ($K{=}10$ pre-floor, $K{=}9$
post-floor): $\bar\rho_{\text{same/release}}{=}0.956$,
$\bar\rho_{\text{cross}}{=}0.625$, $\Delta_{0.30}{=}0.331$
[case 95\% CI $0.300, 0.367$], and $\Delta_{\mathrm{no\,floor}}{=}0.367$. The
sign and magnitude track the printed Table~\ref{tab:main} primary
($K{=}10$, $\bar\rho_{\text{same}}{=}0.959$, $\bar\rho_{\text{cross}}{=}0.639$,
$\Delta_{0.30}{=}0.321$, $\Delta_{\mathrm{no\,floor}}{=}0.355$); the case
intervals overlap and exclude zero. CLIP-L/14 inclusion is therefore not the
driver of the ACDC primary signal. Source:
\texttt{results/\_merged/diagnostics/acdc\_clipl14\_excluded.json}.
...
\begin{table}[!htbp]
  ...
  $\beta_{\mathrm{same}}$ is the coefficient on the primary same-group indicator
...
\begin{table}[!htbp]
  ...
  primary cross-group pairs. Joint fail same/cross is the pairwise joint-failure
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{l c c c c}
    \toprule
    Task & $\tau$ & Mono & Diverse & $\Delta_{\mathrm{rec}}$ \\
    \midrule
...
At the strictest tested threshold ($\tau{=}0.90$), the diverse advantage in conditional recovery vanishes and reverses on RIGA Cup ($\Delta{=}-0.001$) and ACDC LV ($\Delta{=}-0.007$), while remaining positive on Kvasir ($+0.005$) and slightly negative on BraTS ($-0.003$). The diverse-pool benefit is concentrated in the moderate-failure regime ($\tau\in[0.70,0.80]$), consistent with the primary correlation gap predicting reduced \emph{but not zero} coverage benefit at extreme thresholds.
...
\begin{table}[!htbp]
  \caption{\textbf{Kvasir-SEG secondary split diagnostic.} This generic 8-bundle summary uses a 1{,}000-image, 700/300 train/test split and is not the 11-bundle, 880/120 Kvasir row in Table~\ref{tab:main}. Seven of eight generic encoder bundles produce functional segmentation (Dice${>}0.40$); CLIP ViT-L/14 is non-functional and excluded from cross-family analysis. MedSAM-only traces are reported separately as an encoder-probe summary in Appendix~\ref{app:medsam-note}; no RETFound Kvasir trace is included in the released artifact bundle.}
  \label{tab:kvasir-secondary}
...
\begin{table}[!htbp]
  ...
  \end{tabular*}
  \\[2pt]
  {\footnotesize $^{*}$ Ratio = Mono / Matched-4 PERM. \textbf{Bold} = preferred symmetric-permuted estimator (24 seed-to-FM tuples). Matched-4 seed 42 is a single-seed diagnostic. Matched-4 INDEP ($4^4{=}256$ iid tuples) produces values within $\pm 1.5\%$ of PERM on every row and is omitted for compactness; the full table is a secondary diagnostic rather than the \texttt{results/\_merged} primary scorer output. Supp.\ $\checkmark$ iff $N\cdot\mathbb{E}[\mathrm{AF}_{\mathrm{ind}}]{\geq}5$ for the PERM estimator. Mono lift values span four orders of magnitude ($1.4$--$4{,}450{\times}$); precision is reported to the data's underlying decimals and is not uniform across the lift column.}
\end{table}
...
\textbf{RETFound} is a ViT-L/16 MAE pretrained on 1.6M retinal images~\citep{retfound2023}. The code path builds the RETFound-MAE CFP ViT-L/16 trunk and expects the gated HuggingFace checkpoint \texttt{YukunZhou/RETFound\_mae\_natureCFP} or a user-specified checkpoint path; the official upstream project is \texttt{rmaphoh/RETFound\_MAE}. RETFound MAE pretraining uses ImageNet $[0.485, 0.456, 0.406]/[0.229, 0.224, 0.225]$ normalisation; our frozen-probe adaptation would drop the CLS token and reshape the $196{=}14{\times}14$ patch tokens into a spatial $14{\times}14$ feature map of dimension 1024. That patch-token extraction is not RETFound's official global/CLS-style classification fine-tuning path. Because the gated checkpoint was unavailable in the release environment, RETFound cells are not reported and no RETFound result supports the paper's empirical claims.
...
\section{Scope-only exploratory probes}
\label{sec:acdc-detail}
\label{sec:adaptation}
\label{app:hetlora}
\label{app:crossarch}

\emph{Note on CLIP-L on ACDC.} The secondary 8-FM ACDC diagnostic discussed in this section uses a different split where ResNet-50, ResNet-18, and CLIP ViT-L/14 collapse below the Dice${\geq}0.30$ floor (Dice $0.099$, $0.230$, and $0.007$ respectively); the \emph{primary} ACDC LV pool of Table~\ref{tab:main} (post-floor 10-bundle subset of the 11-bundle source pool, with only ResNet-18 dropped) retains CLIP ViT-L/14 because it passes the Dice floor on the primary split. Different splits, different functional outcomes for the borderline bundles.

This block separates released diagnostics from exploratory mechanism probes
without released trace artifacts or aggregate JSONs. Only released traces or
released aggregate JSONs are used as numerical support for the paper's claims.
The shared reading is descriptive: same-backbone perturbations can reduce
correlation, but the released diagnostics do not make same-backbone pools behave
like the primary cross-family references.
...
secondary splits and source prediction caches outside the released artifact. The
released no-pretraining evidence is limited to the CNN random-init
...
aggregate summary files outside the released artifact. We do not tabulate
...
\textbf{Finding.} The UNet-skip head improves mean Dice substantially over several weak $1{\times}1$ probe cells, and within-backbone seed-pair $r$ stays positive on every cell (range $0.426$--$0.979$, with the DINOv2-B RIGA Disc outlier at $0.426$ a single high-variance pair).

\emph{Caveat on cross-bundle comparison.} The cross-bundle Pearson references printed in the table caption ($0.361$ for RIGA Cup, etc.) are computed from the primary $1{\times}1$-conv pool with $4$ seeds per bundle and many bundle pairs; the UNet-skip cells use $2$ seeds and a different decoder. Comparing within-backbone UNet-skip seed-pair $r$ (single Pearson per cell) directly to those cross-bundle references mixes a decoder shift, a seed-count shift, and a pair-mixture shift. We therefore do not claim the UNet-skip same-backbone correlations sit ``above the cross-bundle reference'' numerically.

\emph{Decoder-matched check, all five tasks.} As a decoder-matched reference we recompute UNet-skip within-FM seed-pair $r$ and cross-bundle $r$ on the released per-case Dice (seeds $42/43$, intersection-aligned over the released test cases; cross-pairs restricted to within-split because original-5 and e1-4 expansions use disjoint test splits). Released values (Source: \texttt{results/\_merged/diagnostics/unetskip\_decoder\_matched\_5tasks.json}):
\begin{center}\footnotesize
\begin{tabular}{lcccc}
\toprule
Task & within $\bar r$ ($n$) & cross $\bar r$ ($n$) & decoder-matched gap & 1$\times$1 primary $\Delta_{0.30}$ \\
\midrule
Kvasir       & $0.867$ ($9$) & $0.689$ ($72$) & $0.178$ & $0.261$ \\
ACDC LV      & $0.940$ ($9$) & $0.866$ ($72$) & $0.074$ & $0.321$ \\
BraTS fg     & $0.925$ ($5$) & $0.860$ ($20$) & $0.064$ & $0.315$ \\
RIGA Cup     & $0.920$ ($9$) & $0.851$ ($32$) & $0.068$ & $0.473$ \\
RIGA Disc    & $0.770$ ($9$) & $0.545$ ($72$) & $0.226$ & $0.638$ \\
\bottomrule
\end{tabular}
\end{center}
The decoder-matched gap is $14$--$68\%$ of the $1{\times}1$-conv primary post-floor gap (mean attenuation $\sim$32\%). The absolute level of cross-bundle correlation rises sharply when the decoder is matched (e.g., RIGA Cup cross $\bar r$ goes $0.361 \to 0.851$ when the cross reference is recomputed under UNet-skip), while the same/cross gap shrinks. A full nnU-Net v2 $5$-fold audit on RIGA Cup is reported in Appendix~\ref{app:nnunet}.
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\linewidth}{p{0.16\linewidth}Yp{0.12\linewidth}cccc}
    ...
    \midrule
    Baseline (App.~\ref{app:nnunet}) & nnU-Net 2D, $100$ epochs, separate split & RIGA Cup & $0.972$ & $0.925$ & $0.047$ & $[0.030, 0.069]$ \\
...
\textbf{Summary.} Across the five held-out 2D binary derivatives we tested, two rows give substantively positive gaps (Hippocampus $\Delta{=}0.491$, ISIC-2018 $\Delta{=}0.406$), one is tiny-$N$ positive (Prostate, $n{=}6$, $\Delta{=}0.146$), one gives a near-zero gap (Spleen $\Delta{=}0.003$), and one fails the functional floor (Heart, $0/11$ functional FMs). The held-out evidence \emph{supports} rather than confirms the primary 2D rows and is consistent with task-specific structure modulating the gap magnitude.
...
\begin{table}[!htbp]
  ...
  \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lcccccc}
    \toprule
    Task & $N$ & Func./avail. & Within & Cross & $\Delta$ & $M_{\mathrm{eff}}$ \\
    \midrule
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{c|ccc|ccc}
    ...
    \midrule
    2$^{\dag}$ & 0.000 & 0.000 & $+0.00$ & 0.000 & 0.000 & $+0.00$ \\
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lcc}
    ...
    (R3) Max $M_{\mathrm{eff}}$                 & 0.011$\,\pm\,$0.011 & 0.327$\,\pm\,$0.100 \\
    (R4) Quality-gated max $M_{\mathrm{eff}}$   & 0.016$\,\pm\,$0.014 & 0.381$\,\pm\,$0.112 \\
    (R5) Diverse-by-arch (2 ViT $+$ 2 CNN)    & 0.096$\,\pm\,$0.017 & 0.445$\,\pm\,$0.036 \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{Held-out estimator note.} Split-stability and alternate-estimator numerical summaries without released aggregate files are omitted from the numeric evidence. This section therefore relies on the released 11-bundle held-out aggregate and the released operational JSONs listed above.
...
RIGA Cup full fine-tuning epoch-sweep traces and aggregate summaries are outside
the released artifact. We therefore do not tabulate those values or use them as
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\linewidth}{p{0.30\linewidth}c p{0.16\linewidth}Y}
    ...
    UNet-skip decoder (9 backbones, 5 tasks expansion)     & $0.426$--$0.979$                 & heterogeneous                & Appendix~\ref{app:unet}, Tab.~\ref{tab:unet} \\
    Fold-reslicing cross-bundle check                      & $0.315$--$0.565$                 & split-stable                 & Appendix~\ref{app:nnunet}; released summary \\
    MC dropout $p{=}0.1$--$0.4$                            & $0.89$--$0.95$                   & $-0.02$ to $-0.10$           & Appendix~\ref{app:m22} \\
    Same-backbone bootstrap-bagging                        & $0.85$--$0.92$                   & $-0.05$ to $-0.12$           & Fig.~\ref{fig:phase3} \\
    RIGA cross-vendor shift                                & OOD w/c $0.743/0.390$             & gap $0.353$; $\Delta_{\mathrm{gap}}{-}0.024$ & Appendix~\ref{app:distshift} \\
    Decoder capacity sweep                                 & $\sim 0.69$                      & $\sim -0.27$                 & Appendix~\ref{app:m3} \\
    \midrule
...
exact aggregate source is outside the released artifact. We therefore omit the
...
\begin{table}[!htbp]
  \caption{\textbf{Support-limit examples for joint-failure lift.} Extreme lifts are shown to explain instability, not to support a primary claim. Lifts $\ge 50\times$ are descriptive (support-limited, wide noise envelope) and are \emph{not} primary evidence.}
...
\label{app:3dfm}
This section provides scope-boundary context. Its per-case outputs and scorer version are not included in the supplementary artifact, so the pilot is not used as primary evidence for Table~\ref{tab:main}, the abstract, or the released artifact claims. It motivates why we avoid generalizing the frozen 2D FM-pool result to 3D segmentation-architecture pools.
...
The main-text Section~\ref{sec:main} reports a cross-fitted \emph{ordinal residualised statistic} $\tilde{\Delta}$ on all five primary rows. Because residualisation subtracts shared per-case difficulty signal from both pair members, $\tilde{\Delta}$ is \emph{not} a Pearson-scaled $\Delta$ and its CI endpoints can exceed $1$ (e.g.\ Kvasir $[0.895, 1.008]$ below). $\tilde{\Delta}$ is read only as a sign-stability check on the primary $\Delta$; its numerical values are not comparable to the raw $\Delta$ or to any value on the canonical correlation scale. Three distinct residualisation procedures appear in this appendix and we tabulate them in order of provenance: (i) the \emph{naive} all-bundle null, included only as a leakage warning; (ii) the \emph{random A-vs-B family-split holdout} on Cup and ACDC (the existing \texttt{r7\_item\_difficulty\_holdout.json} diagnostic, Table~\ref{tab:diffleak} below); and (iii) the \emph{cross-fitted broad-family-leaveout} on all five primary rows (Table~\ref{tab:diffleak-crossfit}, the headline 5-row diagnostic referenced in the main text). The cross-fitted procedure keeps the $\tilde{\Delta}$ sign positive on every primary row (Kvasir $[0.895, 1.008]$, ACDC LV $[0.950, 1.000]$, BraTS fg $[0.910, 0.932]$, RIGA Cup $[0.782, 0.882]$, RIGA Disc $[0.805, 0.920]$). The held-out-partition table below is retained as the historical Cup/ACDC-only diagnostic from \texttt{r7\_item\_difficulty\_holdout.json}.

\textbf{Cross-fitted broad-family-leaveout (headline, all five rows).} For each trace, item difficulty is estimated from traces outside that trace's broad upstream family, the trace is residualised by OLS against this leave-out estimator, and the Table~\ref{tab:main} same/cross rule is recomputed on residuals to yield $\tilde{\Delta}$. The sign of $\tilde{\Delta}$ is positive on all five primary rows. The CIs of $\tilde{\Delta}$ are not comparable to raw-$\Delta$ CIs and the magnitudes should not be ranked against raw $\Delta$ values; $\tilde{\Delta}$ is reported only as a sign-stability check.

\begin{table}[!htbp]
  \caption{\textbf{Cross-fitted item-difficulty ordinal residualised statistic $\tilde{\Delta}$, all five primary rows.} Each trace's item-difficulty regressor is estimated using only traces from broad upstream families \emph{other than} the trace's own family. $\tilde{\Delta}$ is \emph{not} a Pearson-scaled $\Delta$ (CI endpoints can exceed $1$) and is reported only for sign stability of the primary $\Delta$; its magnitudes are not comparable to raw $\Delta$ values. Source: \texttt{results/\_merged/diagnostics/item\_difficulty\_robustness.json} (schema \texttt{fmpool\_item\_difficulty\_robustness\_v1}); $B{=}2{,}000$ case bootstrap.}
  \label{tab:diffleak-crossfit}
  \centering\footnotesize
  \setlength{\tabcolsep}{3pt}
  \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lcccc}
    \toprule
    Task & Raw $\Delta$ 95\% CI & Ordinal $\tilde{\Delta}$ 95\% CI & Sign & Functional pool \\
    \midrule
...
\textbf{Random A-vs-B family-split holdout (two-task null).} To remove the leakage we split the 8 bundles into halves
...
The released diagnostic JSON records a fixed set of 10 held-out partitions and reports the mean and range of the resulting family-grouped $\Delta$ values across those partitions. This procedure is procedurally distinct from the cross-fitted broad-family-leaveout above: it does a random within-task split of the panel rather than a leave-one-broad-family-out cross-fit, and it covers only Cup and ACDC.

\begin{table}[!htbp]
  \caption{\textbf{Two-task item-difficulty residualisation diagnostics (random A-vs-B family-split holdout).} The naive null is included only as a leakage warning. Raw values use this diagnostic's family grouping and are not Table~\ref{tab:main} values; the held-out-partition null avoids leakage on Cup/ACDC only. This is procedurally distinct from the cross-fitted broad-family-leaveout in Table~\ref{tab:diffleak-crossfit}.
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.95\linewidth]{../figures/fig_diffleak_procedures.pdf}
  \caption{\textbf{Three item-difficulty residualisation procedures on aligned per-case Dice.} Each procedure operates on the same released traces but uses a different difficulty regressor. \emph{Raw} (dark grey) is Pearson $\Delta$ without residualisation, recomputed per task at the audit's family grouping. \emph{Cross-fitted broad-family-leaveout} (blue, all five primary rows) estimates each trace's difficulty regressor from traces outside the trace's own broad upstream family; the residualised gap CIs are taken from \texttt{item\_difficulty\_robustness.json} (\texttt{cross\_fitted\_item\_difficulty\_floor\_0.30.bootstrap}). \emph{Held-out family-split} (orange, Cup and ACDC only) is the random A-vs-B split-holdout from \texttt{r7\_item\_difficulty\_holdout.json}; mean across released partitions. \emph{Naive (leaked, illustrative)} (red hatched) is the all-bundle null whose mechanical leakage is documented in the prose above. The cross-fitted bars are the procedure referenced in the main text Section~\ref{sec:main}; the held-out family-split bars are a procedurally distinct two-task companion. Error bars are 95\% case-bootstrap intervals where reported. Source script: \texttt{code/scripts/build\_diffleak\_procedures\_figure.py}.}
  \label{fig:diffleak-procedures}
\end{figure}
...
\section{Correlation-equivalent effective pool size $M_{\mathrm{eff}}$}
\label{app:meff}
...
correlation $\rho_{\mathrm{fail}}$, the variance of the per-case mean indicator is
$\sigma^2_F [1 + (M-1)\rho_{\mathrm{fail}}] / M$ where $\sigma^2_F$ is the marginal
variance. Equating this with the variance of a hypothetical independent pool of
size $M_{\mathrm{eff}}$, i.e.\ $\sigma^2_F / M_{\mathrm{eff}}$, gives
\[
  M_{\mathrm{eff}} \;=\; \frac{M}{1 + (M-1)\,\rho_{\mathrm{fail}}}.
\]
...
the audit; it is \emph{not} a literal independent-member count. The reported $M_{\mathrm{eff}}$ values follow algebraically from the primary correlation values in Table~\ref{tab:main}; we report them as a compact summary of the primary same-vs-cross gap, not as additional independent evidence.

\begin{table}[!htbp]
  \caption{\textbf{Threshold-specific effective pool-size diagnostic at $\tau{=}0.85$.}
  $M_{\mathrm{eff}}$ uses an equicorrelation approximation for binary failure
  ...
  $\rho_{\mathrm{fail}}$ over that released enumeration and gives a conservative
  lower $M_{\mathrm{eff}}$ readout for the most redundant valid tuple. Undefined
  ...
  \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lccccccc}
    \toprule
    Task & Func. bundles & Mono mean$^{\dag}$ & Diverse $\bar\rho_{\mathrm{fail}}$ & $\rho_{\max}$ & Mean $M_{\mathrm{eff}}$ & $M_{\mathrm{eff}}(\rho_{\max})$ & Valid tuples \\
    \midrule
...
$M_{\mathrm{eff}}\approx2$--3 under the mean equicorrelation readout, not four.
...
failure threshold, so we use $M_{\mathrm{eff}}$ as a dependence readout, not as a
literal deployment objective.

\paragraph{Pairwise $\rho_{\mathrm{fail}}$ heterogeneity disclosure (equicorrelation caveat).} The $M_{\mathrm{eff}}$ formula assumes constant pairwise correlation across member pairs (equicorrelated pool). In practice the per-pair $\rho_{\mathrm{fail}}$ varies across the bundles in each task. From the \texttt{m\_eff/*.json} outputs, mono-pool $\rho_{\mathrm{fail}}$ across the per-task functional bundles has mean / SD / range:
Kvasir mean $0.865$, SD $0.103$, range $[0.601, 0.948]$ ($n{=}11$);
ACDC LV mean $0.847$, SD $0.116$, range $[0.518, 0.931]$ ($n{=}10$);
BraTS foreground mean $0.860$, SD $0.111$, range $[0.553, 0.958]$ ($n{=}10$);
RIGA Cup mean $0.833$ over 7 defined mono pools, and RIGA Disc mean $0.812$ over 10 defined mono pools. Undefined rows occur when an FM's $\tau{=}0.85$ failure indicator is constant, giving zero-variance Pearson. Diverse 4-of-$K$ compositions span a wider $\rho_{\mathrm{fail}}$ distribution, which is why Table~\ref{tab:meff} reports both means and the released-enumeration $\rho_{\max}$ rather than selecting a single representative pool.
...
\textbf{Implication for $M_{\mathrm{eff}}$.} When the binary failure thresholds
preserve the same broad ordering as the continuous Dice scores, $M_{\mathrm{eff}}$
is monotone decreasing in the observed $\rho_{\mathrm{fail}}$ by definition. The
empirical diagnostics therefore suggest the descriptive ordering
$M_{\mathrm{eff}}^{\text{within-FM}} \leq M_{\mathrm{eff}}^{\text{same-arch}} \leq M_{\mathrm{eff}}^{\text{cross-arch}}$
...
$M_{\mathrm{eff}}$ range; diverse pools that cross both architecture and
...
outside the released artifact, so it is not tabulated as released
evidence here.
...
\begin{table}[!htbp]
  ...
  raw NPZ prediction caches are outside the released artifact.}
...
\begin{table}[!htbp]
  ...
  same-bundle seeds. The \texttt{Diagnostic cross} column is not the
...
\section{Broader impacts and responsible AI}
\label{app:broader-impacts}

This is a retrospective audit of evaluation evidence; it does not propose, deploy, or recommend any clinical AI system. The released artifact comprises per-case Dice scalars, deterministic hashed alignment IDs, scorer code, and metadata. No raw images, no per-pixel predictions, no checkpoints, and no demographic attributes are redistributed.

\textbf{Intended use.} Recompute the audit statistics of Table~\ref{tab:main}, run sensitivity diagnostics, and reuse the trace artifact for related evaluation-protocol research. The audit is scoped to retrospective benchmarks; results are not population-level inferences over future encoders, datasets, recipes, or clinical workflows.

\textbf{Foreseeable misuse.} The case-level traces could in principle be combined with publicly available benchmark images to attempt re-identification. We mitigate this by releasing only deterministic 16-hex hash alignment IDs (e.g., \texttt{riga\_magrabia\_<hash>}) without source filenames, by not redistributing pixel-level outputs, and by stating the prohibited-use list in DATA\_CARD.md (no re-identification, profiling, protected-attribute inference, clinical diagnosis, or deployment decisions).

\textbf{Patient privacy and PHI.} All upstream datasets (Kvasir-SEG, ACDC, BraTS 2024 GLI, RIGA+, ISIC 2018, MSD) are publicly released under their respective licenses with their own de-identification procedures. The derived scalar traces and hashed IDs preserve those de-identification guarantees and do not introduce new PHI exposure.

\textbf{Demographic and fairness limitations.} The released benchmarks lack standardised demographic annotations; we do not perform subgroup or fairness analyses, and we do not claim subgroup robustness. The audit is a redundancy diagnostic over fixed pools, not a fairness diagnostic.

\textbf{Environmental footprint.} The released audit is recomputable from trace JSONs in CPU minutes-scale (Appendix~\ref{app:release-manifest}); no GPU is required for the released scorer pipeline. Upstream feature-extraction and decoder training were performed on shared academic GPU infrastructure; the released artifact does not require that compute to be reproduced.

\textbf{Licensing.} Authored code under Apache-2.0; derived traces inherit the most restrictive upstream license that applies (e.g., MAE- or RETFound-derived traces are CC BY-NC 4.0 research-only). DATA\_CARD.md gives the per-upstream license matrix.
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\linewidth}{p{0.25\linewidth}p{0.24\linewidth}Y}
    ...
    plus storage for upstream data and features & Feature extraction is
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{p{0.25\linewidth}p{0.31\linewidth}p{0.36\linewidth}}
    ...
    \texttt{nnunet/*.json}, \texttt{nnunet\_case\_riga\_2d\_100ep/*.json} &
    Released summaries plus the RIGA case-identical nnU-Net per-model traces;
    other full-pipeline comparators remain aggregate or task-level only. \\
    \bottomrule
  \end{tabular}
\end{table}
...
\texttt{per\_case\_dice} tree, the bundle includes released trace sets for
...
from the upstream provider. Larger derived caches and more detailed split manifests are
