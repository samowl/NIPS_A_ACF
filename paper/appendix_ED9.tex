...
\label{sec:threshold}
\label{app:tau-sweep}
ALL-fail rates at $P_{25}$, $P_{33}$, and $P_{50}$ thresholds for the RIGA Cup data in Table~\ref{tab:threshold}; absolute-threshold lift with support flags is in Appendix~\ref{sec:matched-app}. The pattern persists across the RIGA-Cup thresholds shown; broader threshold robustness is descriptive and many lift thresholds are support-limited (Appendix~\ref{sec:matched-app}).
