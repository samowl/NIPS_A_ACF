...
\paragraph{Pending case-identical 1000-epoch RIGA run.} A companion 1000-epoch RIGA \emph{case-identical} run (Magrabia-source held-out test cases) is in progress; on completion the row will be inserted into Table~\ref{tab:w1-scope} following the procedure documented in \texttt{reports/12\_pending\_1000ep\_insertion\_points.md}. The current 1000-epoch RIGA Cup row in this section uses the separate $520/224$ task-level split, not the Magrabia-source held-out split.
